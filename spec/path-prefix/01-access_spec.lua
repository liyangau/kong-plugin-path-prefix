...
local strategies = helpers.all_strategies ~= nil and helpers.all_strategies or
                       helpers.each_strategy

for _, strategy in strategies() do
  describe("Plugin: " .. PLUGIN_NAME .. " (access) [#" .. strategy .. "]",
           function()
    local client
    local db_strategy = strategy ~= "off" and strategy or nil
    lazy_setup(function()
      local bp = helpers.get_db_utils(db_strategy, {}, {PLUGIN_NAME})
      local service1 = bp.services:insert{url = echo_url}
      ...
      bp.plugins:insert{
        route = {id = route1.id},
        name = PLUGIN_NAME,
        config = {path_prefix = "/service-with-plugin-route"}
      }

      bp.plugins:insert{
        route = {id = route2.id},
        name = PLUGIN_NAME,
        config = {path_prefix = "/service-with-plugin-route", escape = false}
      }
    ...
    describe("access test", function()
      ...
      it("does not escape dash when escape option is false", function()
        local res = assert(client:send{
          method = "GET",
          path = "/service-with-plugin-route/foobar",
